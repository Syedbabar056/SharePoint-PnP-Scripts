...
Function Clone-PnPUser {
    ...
    #Handling inactive/ missing users 
    if (-not $SourceUser) { 
      #$SourceUser = New-PnPUser -LoginName $SourceUserEmail 
      Write-Host "⚠️ Source user '$SourceUserEmail' not found or inactive. Skipping site $SiteURL." -ForegroundColor Red
      return
    }
    if (-not $TargetUser) { 
      #$TargetUser = New-PnPUser -LoginName $TargetUserEmail 
       Write-Host "⚠️ Target user '$TargetUserEmail' not found or inactive. Skipping site $SiteURL." -ForegroundColor Red
      return 
    }
    #cloning validation check for Admin role
